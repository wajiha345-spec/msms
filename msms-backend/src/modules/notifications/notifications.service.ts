...
// overdue installments, installments due within 7 days, overdue CRM
// follow-ups, plus (below) an upcoming license installment payment — NOT
// the same thing as overdue/dueSoonInstallments above, which are the
// shop's own customers buying on installment; upcomingLicenseInstallment is
// the shop paying its own MSMS license in installments (see
// licenseInstallments.service.ts).
export async function getAttentionItems(shopId: string) {
  const now = new Date();
  const weekOut = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);

  const [overdueInstallments, dueSoonInstallments, overdueFollowUps, licensePlan] = await Promise.all([
    // Reads SaleInstallment (the 1st/2nd/3rd schedule — see
    // sales.service.ts:buildInstallmentSchedule / prisma/schema.prisma),
    // not Sale directly, so the installment number is known and every
    // existing installment sale (backfilled at migration time) is covered.
    prisma.saleInstallment.findMany({
      where: { status: 'PENDING', dueDate: { lt: now }, sale: { shopId } },
      orderBy: { dueDate: 'asc' },
      include: { sale: { select: { invoiceNo: true, customerName: true, customerPhone: true } } },
    }),
    // "Due soon" — not yet overdue, but due within 7 days. Same computed-not-
    // persisted approach as overdueInstallments above.
    prisma.saleInstallment.findMany({
      where: { status: 'PENDING', dueDate: { gte: now, lte: weekOut }, sale: { shopId } },
      orderBy: { dueDate: 'asc' },
      include: { sale: { select: { invoiceNo: true, customerName: true, customerPhone: true } } },
    }),
  ...
  return {
    overdueInstallments: overdueInstallments.map((i) => ({
      saleId: i.saleId, invoiceNo: i.sale.invoiceNo, customerName: i.sale.customerName, customerPhone: i.sale.customerPhone,
      pendingAmount: i.amount, dueDate: i.dueDate, installmentNumber: i.installmentNumber,
    })),
    dueSoonInstallments: dueSoonInstallments.map((i) => ({
      saleId: i.saleId, invoiceNo: i.sale.invoiceNo, customerName: i.sale.customerName, customerPhone: i.sale.customerPhone,
      pendingAmount: i.amount, dueDate: i.dueDate, installmentNumber: i.installmentNumber,
    })),
