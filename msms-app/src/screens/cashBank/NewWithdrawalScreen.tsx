...
export default function NewWithdrawalScreen() {
  ...
  function validate() {
    ...
    if (!fromAccount) e.fromAccount = 'Please select which account the money leaves';
    if (!toAccount)   e.toAccount = 'Please select where the money went';
  ...
  async function handleSubmit() {
    ...
    setLoading(true);
    try {
      await cashBankApi.recordWithdrawal({
        fromAccountId: fromAccount!.id,
        toAccountId:   toAccount!.id,
        amount:        Number(amount),
        memo:          memo.trim() || undefined,
      });
      Alert.alert('Withdrawal Recorded ✓', undefined, [{ text: 'OK', onPress: () => navigation.goBack() }]);
    } catch (e: any) {
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>New Withdrawal</Text>
        <View style={{ width: 60 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled">
        ...
        <AccountPicker
          label="Destination (where the money went)"
          value={toAccount}
        ...
        <Input
          label="Memo (optional)"
          placeholder="e.g. Owner drawings"
          value={memo}
          onChangeText={setMemo}
        />

        <Button label="Record Withdrawal" onPress={handleSubmit} loading={loading} style={{ marginTop: 8 }} />
