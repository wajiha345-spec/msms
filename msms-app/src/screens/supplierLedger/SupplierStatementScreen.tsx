...
import { supplierLedgerApi, SupplierStatement, SupplierLedgerTxn } from '../../api/supplierLedger';
...
export default function SupplierStatementScreen() {
  ...
  function renderItem({ item }: { item: SupplierLedgerTxn }) {
    const date = new Date(item.date);
    const isPurchase = item.type === 'PURCHASE';
    return (
      <View style={styles.row}>
        <View style={{ flex: 1 }}>
          <Text style={styles.rowRef}>{isPurchase ? item.ref : `Payment (${item.ref})`}</Text>
          <Text style={styles.rowDate}>{date.toLocaleDateString('en-PK')}</Text>
        </View>
        <View style={styles.rowRight}>
          <Text style={[styles.amount, isPurchase ? styles.debitAmount : styles.creditAmount]}>
            {isPurchase ? '+' : '-'}Rs {Math.abs(item.amount).toLocaleString()}
          </Text>
  ...
  return (
    <View style={styles.container}>
      ...
      {statement.outstanding > 0 && (
        <View style={styles.actionRow}>
          <Button
            ...
            onPress={() => navigation.navigate('RecordSupplierPayment', { phone, name })}
          />
        </View>
      )}
      ...
      <FlatList
        ...
        keyExtractor={(item, i) => `${item.type}-${item.purchaseId}-${i}`}
